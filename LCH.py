...
def generate_lch_grid(hex_color, c_min=10, step_l=5, step_c=5):
    ...
    # Ile kroków w lewo (przyciemnianie) i w prawo (rozjaśnianie)
    ...
    for r in range(rows):
        C = max(c_min, C0 - r*step_c)  # desaturacja w pionie
        for i in range(-cols_left, cols_right + 1):
            L = L0 + i*step_l
            L = max(0, min(100, L))  # ograniczenie do 0-100
...
# --- STREAMLIT INTERFEJS ---
st.title("Siatka kolorów LCH (poprawiona)")
st.write("Wprowadź kolor bazowy HEX, a program wygeneruje siatkę z rozjaśnianiem w prawo, przyciemnianiem w lewo i desaturacją w dół.")
